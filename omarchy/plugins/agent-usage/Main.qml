...
Item {
  ...
  function updateCommand(kind, agentIds) {
    var command = [root.home + "/.config/omarchy/plugins/agent-usage/collectors/usage-update"]
  ...
  // Display-name overrides: the packaged collectors write the full product
  // name ("Claude Code"); the panel shows a shorter label.
  function displayName(id, fallbackName) {
    var overrides = { "claude": "Claude" }
    return overrides[String(id)] || String(fallbackName || id)
  }
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      providerName: displayName(record.id, record.name),

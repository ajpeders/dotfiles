...
-- ====== Living-room TV ======
-- 55" Insignia (EDID reports make as "Ines GmbH", model NS55DF710NA21).
-- 4K@60 preferred. `hypr/scripts/livingroom-mode.sh` toggles every
...
-- Runtime overrides generated by livingroom-mode.sh. Loading them as part of
-- the normal config avoids the Hyprland 0.56.2 crash in `hyprctl eval` and the
-- native Lua parser's rejection of `hyprctl keyword`.
local state_home = os.getenv("XDG_STATE_HOME") or (os.getenv("HOME") .. "/.local/state")
local livingroom_mode = state_home .. "/dotfiles/livingroom-mode.lua"
local livingroom_file = io.open(livingroom_mode, "r")
if livingroom_file then
    livingroom_file:close()
    dofile(livingroom_mode)
end

...
HYPR_SNIPPET="$HYPR_DIR/config/omarchy-shell.conf"
SNIPPET_SOURCE_LINE="source = ~/.config/hypr/config/omarchy-shell.conf"
...
phase_hyprland() {
    ...
    if [ ! -f "$HYPR_CONF" ]; then
        print_info "No $HYPR_CONF; skipping the autostart wiring."
        print_info "Add this to whatever config your compositor does read:"
        print_info "  exec-once = $LAUNCHER"
        return 0
    fi
...
EOF
    ...
    if grep -qF "$SNIPPET_SOURCE_LINE" "$HYPR_CONF"; then
        print_status "hyprland.conf already sources it"
    else
        printf '\n%s\n' "$SNIPPET_SOURCE_LINE" >> "$HYPR_CONF"
        print_status "Added the source line to hyprland.conf"
    fi
}
...
report_conflicts() {
    ...
    local autostart="$HYPR_DIR/config/autostart.conf"
    if [ -f "$autostart" ]; then
        grep -q 'noctalia-shell.service' "$autostart" &&
            print_info "Comment out the noctalia-shell exec-once in $autostart"
        if grep -q 'hypridle-power.sh' "$autostart"; then
            print_info "Comment out hypridle-power.sh in $autostart -- the shell's idle"
        ...
        grep -qE 'polkit-gnome|cliphist' "$autostart" &&
            print_info "polkit-gnome and cliphist are redundant now; the shell ships both."
    fi

    print_info "Nothing binds the menu yet. In hypr/config/keybinds.conf:"
    print_info "  bind = SUPER, SPACE, exec, $OMARCHY_ROOT/bin/omarchy-menu"
    ...
    echo
    print_info "Do NOT run 'omarchy theme set' here without reading it first: it stages"
    print_info "  themed configs over kitty.conf, foot.ini and alacritty.toml, all of"
    print_info "  which this repo tracks. The shell falls back to its built-in palette"
    print_info "  when no theme is set, so it works fine untheme'd."
}
...
do_check() {
    ...
    if [ -f "$HYPR_CONF" ] && grep -qF "$SNIPPET_SOURCE_LINE" "$HYPR_CONF"; then
        print_status "hyprland.conf sources the autostart snippet"
    else
        print_error "hyprland.conf does not source $HYPR_SNIPPET"; ok=1
    fi
...
do_uninstall() {
    ...
    if [ -f "$HYPR_CONF" ] && grep -qF "$SNIPPET_SOURCE_LINE" "$HYPR_CONF"; then
        local tmp
        tmp="$(mktemp)"
        grep -vF "$SNIPPET_SOURCE_LINE" "$HYPR_CONF" > "$tmp"
        cat "$tmp" > "$HYPR_CONF"
        rm -f "$tmp"
        print_status "Dropped the source line from hyprland.conf"
    fi

    print_info "quickshell was left installed. To go back to Noctalia:"
    print_info "  sudo pacman -Rdd quickshell && paru -S noctalia-qs"
    print_info "and re-enable the exec-once lines in hypr/config/autostart.conf."
}
